							...
							<div id="labCart" {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
								<p id="add_to_cart" class="buttons_bottom_block no-print">
									<button type="submit" name="Submit" class="exclusive">
										 <span><!-- <i class="icons-handbag"></i> --> {if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}{l s='+ Add to cart'}{/if}</span>
									</button>
								</p>
							</div>
		...
		<!--HOOK_PRODUCT_TAB -->
			...
			<section class="page-product-box pos-block lablistproducts laberthemes">
				...
				<div class="block products_block lab-accessories row">
												...
												<div class="actions">
														...
														{if isset($comparator_max_item) && $comparator_max_item}
															<li class="lab-compare">	
																<a class="add_to_compare" 
																	...
																	data-product-name="{$accessory.name|escape:'htmlall':'UTF-8'}"
																	data-product-cover="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'cart_default')|escape:'html'}"
													...
													{if $lab_EnableQv == 1}
															<li class="lab-quick-view">
																<a class="quick-view" href="{$accessory.link|escape:'html':'UTF-8'}"
																	data-id-product="{$accessory.id_product|intval}"
													...
													{if ($accessory.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $accessory.available_for_order && !isset($restricted_country_mode) && $accessory.minimal_quantity <= 1 && $accessory.customizable != 2 && !$PS_CATALOG_MODE}
														{if ($accessory.allow_oosp || $accessory.quantity > 0)}
															{if isset($static_token)}
																<a class="button ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}"
																data-id-product="{$accessory.id_product|intval}"
					...
					<div class="lab_boxnp">
						<a class="prev prevac"><i class="icon icon-angle-left"></i><span></span></a>
						<a class="next nextac"><span></span><i class="icon icon-angle-right"></i></a>
					</div>
					<script>
