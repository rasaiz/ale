...
include_once(_PS_MODULE_DIR_.'labmegamenu/sql/labSampleDataMenu.php');

class Labmegamenu extends Module
{
	...
	public function __construct()
	{
		...
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('Lab Megamenu custom');
		$this->description = $this->l('Show megamenus custom');
	...
	public function install()
	{
		...
		foreach (Language::getLanguages() as $language)
            $tab->name[$language['id_lang']] = $this->l('Manage MegaMenu');
		...
		if (parent::install() &&
			...
			$this->registerHook('actionObjectLanguageAddAfter'))
			{
				...
				$sample_data = new labSampleDataMenu();
	...
	{
		$tab = new Tab((int)Tab::getIdFromClassName('Adminlabmegamenu'));
        $tab->delete();	
	...
	public function displayStatus($id_labmegamenu, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				...
				&changeStatus&id_labmegamenu='.(int)$id_labmegamenu.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	public function displayStatusRow($id_menu, $id_row, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusRow&id_labmegamenu='.$id_menu.'&id_row='.(int)$id_row.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	public function displayStatusSubItem($id_menu, $id_item, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				&token='.Tools::getAdminTokenLite('AdminModules').'
				&changeStatusSubItem&id_labmegamenu='.$id_menu.'&id_item='.(int)$id_item.'" title="'.$title.'"><i class="'.$icon.'"></i></a>';
	...
	{
		$req = 'SELECT wt.`id_labmegamenu` as id_labmegamenu
				FROM `'._DB_PREFIX_.'labmegamenu` wt
				WHERE wt.`id_labmegamenu` = '.(int)$id;
	...
	public function hooklabmegamenu()
	{
		...
		return $this->display(__FILE__, 'labmegamenu.tpl', $this->getCacheId('labmegamenu'));
	}
	...
	public function getRespCategories($id_category = 1, $id_lang = false, $id_shop = false, $level_root = 1, $sub_title = '', $type_icon = 1, $icon = '', $sub_align = '', $spec_class = '')
	{
		...
		if (!empty($is_intersected))
		{
			...
			if (isset($children) && count($children))
				$this->respMenu .= '<i class="icon icon-angle-down"></i>'; 
			$this->respMenu .= '</a>';
		}
