...
class LabMegamenuClass extends ObjectModel
{
	...
	public function getMenus()
	{
		...
		$kq = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT wt.*, wl.*
			FROM '._DB_PREFIX_.'labmegamenu_shop wt
			LEFT JOIN `'._DB_PREFIX_.'labmegamenu_lang` wl ON (wl.`id_labmegamenu` = wt.`id_labmegamenu` AND wt.`id_shop` = wl.`id_shop`)
			WHERE wt.active = 1 AND wl.id_shop = '.$id_shop.' AND wl.id_lang='.$id_lang.' ORDER BY wt.position ASC, wt.id_labmegamenu ASC');
		return $kq;
	}
