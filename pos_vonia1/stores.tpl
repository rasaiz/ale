...
<div class="pos-title" style="padding-top:20px;"><h2><span>{l s='Our stores'}</span></h2></div>

{if $simplifiedStoresDiplay}
	{if $stores|@count}
		<p class="store-title" style="text-align:center;padding-bottom:20px;">
			<strong class="dark">
	    ...
	    <table class="table table-hover">
			...
			{foreach $stores as $store}
				<tr class="store-small">
					<td class="name">
					    <h4 style="color:#65AB74;">{$store.name|escape:'html':'UTF-8'}</h4><br />
						{if $store.has_picture}
					            <img src="{$img_store_dir}{$store.id_store}-large_default.jpg" alt="{$store.name|escape:'html':'UTF-8'}" width="{$largeSize.width}" height="{$largeSize.height}" class="img-thumbnail" />
					    {/if}
					</td>
		            <td class="address">
